...
from .commands import (BaseCommand, AddEdge, AddEdges, AddNode,
                       AddNodeSnapped, AddWNode, ChangeEdgeColor,
                       ChangeEdgeCurve, ChangeNodeType, ChangePhase,
                       MergeNodes, MoveNode, SetGraph, UpdateGraph)
...
def vertices_data() -> dict[VertexType, DrawPanelNodeType]:
    ...
    return {
        VertexType.Z: {
            "text": "Z spider",
            "icon": (ShapeType.CIRCLE, colors["z_spider"])},
        VertexType.X: {
            "text": "X spider",
            "icon": (ShapeType.CIRCLE, colors["x_spider"])},
        VertexType.H_BOX: {
            "text": "H box",
            "icon": (ShapeType.SQUARE, colors["hadamard"])},
        VertexType.Z_BOX: {
            "text": "Z box",
            "icon": (ShapeType.SQUARE, colors["z_spider"])},
        VertexType.W_OUTPUT: {
            "text": "W node",
            "icon": (ShapeType.TRIANGLE, colors["w_output"])},
        VertexType.BOUNDARY: {
            "text": "boundary",
            "icon": (ShapeType.CIRCLE, colors["w_input"])},
        VertexType.DUMMY: {
            "text": "Dummy",
            "icon": (ShapeType.CIRCLE, colors["dummy"])},
    }
...
def edges_data() -> dict[EdgeType, DrawPanelNodeType]:
    return {
        EdgeType.SIMPLE: {
            "text": "Simple", "icon": (ShapeType.LINE, QColor(BLACK))},
        EdgeType.HADAMARD: {
            "text": "Hadamard",
            "icon": (ShapeType.DASHED_LINE, QColor(HAD_EDGE_BLUE))},
    }
...
class EditorBasePanel(BasePanel):
    ...
    def create_side_bar(self) -> None:
        ...
        self.vertex_list = create_list_widget(
            self, vertices_data(), self._vty_clicked,
            self._vty_double_clicked)
        self.edge_list = create_list_widget(
            self, edges_data(), self._ety_clicked,
            self._ety_double_clicked)
    ...
    def update_side_bar(self) -> None:
        populate_list_widget(
            self.vertex_list, vertices_data(), self._vty_clicked,
            self._vty_double_clicked)
        populate_list_widget(
            self.edge_list, edges_data(), self._ety_clicked,
            self._ety_double_clicked)
    ...
    def paste_graph(self, graph: GraphT) -> None:
        ...
        new_verts, _ = new_g.merge(graph.translate(0.5, 0.5))
    ...
    def delete_selection(self) -> None:
        ...
        if len(set(rem_vertices)) > 128:
            ...
            self.undo_stack.push(UpdateGraph(self.graph_view, new_g))
    ...
    def add_vert(self, x: float, y: float, edges: list[EItem]) -> None:
        """Add a vertex at point (x,y). `edges` is a list of EItems
        that are underneath the current position.
        We will try to connect the vertex to an edge.
        """
        cmd: BaseCommand
        snap = self.snap_vertex_edge
        if snap and edges and self._curr_vty != VertexType.W_OUTPUT:
            # Trying to snap vertex to an edge
            for it in edges:
                e = it.e
                g = self.graph_scene.g
                if self.graph_scene.g.edge_type(e) not in (
                        EdgeType.SIMPLE, EdgeType.HADAMARD):
                    continue
                cmd = AddNodeSnapped(
                    self.graph_view, x, y, self._curr_vty, e)
                self.play_sound_signal.emit(SFXEnum.THATS_A_SPIDER)
                self.undo_stack.push(cmd)
                g = cmd.g
                group = QParallelAnimationGroup()
                for e in [next(g.edges(cmd.s, cmd.added_vert)),
                          next(g.edges(cmd.t, cmd.added_vert))]:
                    eitem = self.graph_scene.edge_map[e][0]
                    anim = animations.edge_thickness(
                        eitem, 3, 400,
    ...
    def add_edge(self, u: VT, v: VT, verts: list[VItem]) -> None:
        """Add an edge between vertices u and v. `verts` is a list of
        VItems that collide with the edge.
        If self.snap_vertex_edge is true, then we try to connect `u`
        through all the `vertices` in `verts`, and then to `v`.
        """
        cmd: BaseCommand
        graph = self.graph_view.graph_scene.g
        if vertex_is_w(graph.type(u)) and get_w_partner(graph, u) == v:
            return None
        if (graph.type(u) == VertexType.W_INPUT and
                len(graph.neighbors(u)) >= 2 or
                graph.type(v) == VertexType.W_INPUT and
                len(graph.neighbors(v)) >= 2):
            return None
        if ((graph.type(u) == VertexType.DUMMY and
                graph.type(v) != VertexType.DUMMY) or
                (graph.type(u) != VertexType.DUMMY and
                 graph.type(v) == VertexType.DUMMY)):
            return None

        # We will try to connect all the vertices together in order
        # First we filter out the vertices that are not compatible with
        # the edge. We will be adding two edges, which is not compatible
        # with W_INPUT
        verts = [vitem for vitem in verts if not
                 graph.type(vitem.v) == VertexType.W_INPUT]
        # but first we check if there any vertices that we do want to
        ...
        ux = float(graph.row(u))
        uy = float(graph.qubit(u))
        # Line was drawn from u to v, we want to order vs with the
        # earlier items first.

        def dist(vitem: VItem) -> float:
            vx = float(graph.row(vitem.v))
            vy = float(graph.qubit(vitem.v))
            return (vx - ux) ** 2 + (vy - uy) ** 2
        verts.sort(key=dist)
        ...
        for vitem in verts:
            anim = animations.scale(
                vitem, 1.0, 400,
                QEasingCurve(QEasingCurve.Type.InCubic), start=1.3)
            group.addAnimation(anim)
        self.undo_stack.set_anim(group)
    ...
    def change_edge_curves(
            self, eitem: EItem, new_distance: float,
    ...
    def vert_double_clicked(self, v: VT) -> None:
        ...
        if (graph.type(v) == VertexType.BOUNDARY or
                vertex_is_w(graph.type(v))):
            return None
        ...
        if phase_is_complex:
            ...
            prompt = (
                ...
                "(non-variables are multiples of pi):")
            error_msg = (
                "Please enter a valid input. "
                "(e.g. pi/2, 1/2, 0.25, a+b).")
...
class VariableViewer(QScrollArea):
    ...
    def __init__(self, parent: EditorBasePanel) -> None:
        ...
        vlabel.setSizePolicy(
            QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Fixed)
        self._layout.addWidget(
            vlabel, 0, 0,
            Qt.AlignmentFlag.AlignBottom | Qt.AlignmentFlag.AlignHCenter)
        tlabel = QLabel("Type")
        tlabel.setSizePolicy(
            QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Fixed)
        self._layout.addWidget(
            tlabel, 0, 2,
            Qt.AlignmentFlag.AlignBottom | Qt.AlignmentFlag.AlignHCenter)
        ...
        self._layout.addItem(
            QSpacerItem(0, 0, QSizePolicy.Policy.Expanding,
                        QSizePolicy.Policy.Expanding), 2, 2)
    ...
    def add_item(self, name: str) -> None:
        ...
        is_bool = self.parent_panel.graph.var_registry.get_type(
            name, default=False)
        combobox.setCurrentIndex(1 if is_bool else 0)
        combobox.currentTextChanged.connect(
            lambda text: self._text_changed(name, text))
        item = self._layout.itemAtPosition(2 + self._items, 2)
        assert item is not None  # For mypy
        self._layout.removeItem(item)
        self._layout.addWidget(
            QLabel(f"<pre>{name}</pre>"), 2 + self._items, 0,
            Qt.AlignmentFlag.AlignVCenter | Qt.AlignmentFlag.AlignRight)
        self._layout.addWidget(
            combobox, 2 + self._items, 2, Qt.AlignmentFlag.AlignCenter)
        self._layout.setRowStretch(2 + self._items, 0)
        self._layout.addItem(
            QSpacerItem(0, 0, QSizePolicy.Policy.Expanding,
                        QSizePolicy.Policy.Expanding), 3 + self._items, 2)
    ...
    def _text_changed(self, name: str, text: str) -> None:
        ...
        if isinstance(self.parent_panel, RulePanel):
            self.parent_panel.graph_scene_left.g.var_registry.set_type(
                name, new_type)
            self.parent_panel.graph_scene_right.g.var_registry.set_type(
                name, new_type)
        else:
            self.parent_panel.graph_scene.g.var_registry.set_type(
                name, new_type)


def toolbar_select_node_edge(
        parent: EditorBasePanel) -> Iterator[ToolbarSection]:
    ...
    snap.setToolTip(
        "Snap vertices to the edge beneath them when adding "
        "vertices or edges (f)")
    snap.setShortcut("f")
...
def create_list_widget(
        parent: EditorBasePanel,
        data: (dict[VertexType, DrawPanelNodeType] |
               dict[EdgeType, DrawPanelNodeType]),
        onclick: (Callable[[VertexType], None] |
                  Callable[[EdgeType], None]),
        ondoubleclick: (Callable[[VertexType], None] |
                        Callable[[EdgeType], None])) -> QListWidget:
...
def populate_list_widget(
    ...
    list_widget.itemClicked.connect(
        lambda x: onclick(x.data(Qt.ItemDataRole.UserRole)))
    list_widget.itemDoubleClicked.connect(
        lambda x: ondoubleclick(x.data(Qt.ItemDataRole.UserRole)))
