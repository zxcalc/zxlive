...
from pyzx.utils import (EdgeType, VertexType, FractionLike, get_w_partner,
                        get_z_box_label, set_z_box_label,
                        vertex_is_z_like)
...
from .common import (ET, VT, GraphT, ToolType, get_data, pos_from_view,
                     pos_to_view)
...
class ProofPanel(BasePanel):
    ...
    def __init__(self, graph: GraphT, *actions: QAction) -> None:
        ...
        self.graph_scene.vertex_double_clicked.connect(
            self._vert_double_clicked)
        self.graph_scene.edge_double_clicked.connect(
            self._edge_double_clicked)
    ...
    def _toolbar_sections(self) -> Iterator[ToolbarSection]:
        ...
        self._dummy_edge_icon.setIcon(
            QIcon(get_data("icons/tikzit-tool-edge.svg")))
        ...
        yield ToolbarSection(self.selection, self._dummy_icon,
                             self._dummy_edge_icon, self.magic_wand,
                             exclusive=True)
    ...
    def change_edge_curves(self, eitem: EItem, new_distance: float,
                           old_distance: float) -> None:
        cmd = ProofModeCommand(
            ChangeEdgeCurve(self.graph_view, eitem, new_distance,
                            old_distance),
            self.step_view)
        self.undo_stack.push(cmd)
    ...
    def _vertex_dragged(self, state: DragState, v: VT, w: VT) -> None:
        if state == DragState.Onto:
            ...
            # This function takes a vertex matching function, which we
            # restrict to just match to v and w
            elif pyzx.hrules.match_copy(self.graph,
                                        lambda x: x in (v, w)):
                anims.anticipate_strong_comp(self.graph_scene.vertex_map[w])
            elif editor_actions.pauli_matcher(self.graph,
                                              lambda x: x in (v, w)):
                anims.anticipate_strong_comp(self.graph_scene.vertex_map[w])
        else:
    ...
    def _vertex_dropped_onto(self, v: VT, w: VT) -> None:
        ...
        edges_vw = list(self.graph.edges(v, w))
        if (len(edges_vw) == 1 and
                self.graph.edge_type(self.graph.edge(v, w)) ==
                EdgeType.HADAMARD):
            basicrules.color_change(g, w)
        if pyzx.basicrules.check_fuse(g, v, w):
            pyzx.basicrules.fuse(g, w, v)
            v_map = self.graph_scene.vertex_map
            anim = anims.fuse(v_map[v], v_map[w])
            cmd = AddRewriteStep(self.graph_view, g, self.step_view,
                                 "fuse spiders")
            ...
            result = pyzx.hrules.apply_copy(g, copy_match)
            etab, rem_verts, rem_edges, _ = result
            ...
            pauli_match = editor_actions.pauli_matcher(g,
                                                       lambda x: x in (v, w))
            result = editor_actions.pauli_push(g, pauli_match)
            etab, rem_verts, rem_edges, _ = result
            g.add_edge_table(etab)
            g.remove_edges(rem_edges)
            g.remove_vertices(rem_verts)
            # Determine which vertex is the target
            # (the one being pushed through)
            ...
            move_cmd = ProofModeCommand(
                MoveNode(self.graph_view, [(v, pos[0], pos[1])]),
                self.step_view)
    ...
    def _magic_hopf(self, trace: WandTrace) -> bool:
        if not all(isinstance(item, EItem) for item in trace.hit):
            return False
        edges: list[ET] = [item.e for item in trace.hit]  # type: ignore
        if len(edges) == 0:
            return False
        if not all(edge == edges[0] for edge in edges):
            return False
        source, target = self.graph.edge_st(edges[0])
        source_type = self.graph.type(source)
        target_type = self.graph.type(target)
        edge_type = self.graph.edge_type(edges[0])
        is_hadamard_z = (edge_type == EdgeType.HADAMARD and
                         vertex_is_z_like(source_type) and
                         vertex_is_z_like(target_type))
        is_simple_zx = (edge_type == EdgeType.SIMPLE and
                        vertex_is_z_like(source_type) and
                        target_type == VertexType.X)
        is_simple_xz = (edge_type == EdgeType.SIMPLE and
                        source_type == VertexType.X and
                        vertex_is_z_like(target_type))
        if is_hadamard_z or is_simple_zx or is_simple_xz:
            new_g = copy.deepcopy(self.graph)
            num_edges = len(edges)
            # Remove even number of edges
            if num_edges % 2 != 0:
                num_edges -= 1
            # TODO: This doesn't take into account the global scalar
            # factor.
            for _ in range(num_edges):
                new_g.remove_edge(edges[0])
            # TODO: Add animation for Hopf
            # anim = anims.hopf(edges, self.graph_scene)
            cmd = AddRewriteStep(self.graph_view, new_g, self.step_view,
                                 "Hopf")
            self.undo_stack.push(cmd)
            return True
        return False
    ...
    def _magic_identity(self, trace: WandTrace) -> bool:
        if (len(trace.hit) != 1 or
                not all(isinstance(item, EItem) for item in trace.hit)):
            return False
        ...
        s_is_dummy = self.graph.type(s) == VertexType.DUMMY
        t_is_dummy = self.graph.type(t) == VertexType.DUMMY
        if s_is_dummy or t_is_dummy:
            return False
        ...
        new_g = copy.deepcopy(self.graph)
        v = new_g.add_vertex(vty, row=pos.x()/SCALE,
                             qubit=pos.y()/SCALE)
    ...
    def _magic_slice(self, trace: WandTrace) -> bool:
        ...
        item = filtered[0]
        vertex = item.v
        allowed_types = (VertexType.Z, VertexType.X, VertexType.Z_BOX,
                         VertexType.W_OUTPUT)
        if self.graph.type(vertex) not in allowed_types:
            return False
        ...
        if trace.shift and self.graph.type(vertex) != VertexType.W_OUTPUT:
            ...
            if phase_is_complex:
                ...
                error_msg = ("Please enter a valid input "
                             "(e.g., 1/2, 2, 0.25, 2a+b, 3x^2+4y).")
            text, ok = QInputDialog().getText(
                self, "Choose Phase of one Spider", prompt)
            if not ok:
                return False
            try:
                if phase_is_complex:
                    ...
                    phase = string_to_phase(text, self.graph)
            except ValueError:
        ...
        if start.y() > end.y():
            start, end = end, start
        vertex_row = self.graph.row(vertex)
        vertex_qubit = self.graph.qubit(vertex)
        pos = QPointF(*pos_to_view(vertex_row, vertex_qubit))
        left, right = [], []
        for edge in set(self.graph.incident_edges(vertex)):
            eitems = self.graph_scene.edge_map[edge]
            for eitem in eitems.values():
                # we use the selection node to determine the center of the
                # edge
                epos = eitem.selection_node.pos()
                # Compute whether each edge is inside the entry and exit
                # points
                i1 = (cross(start - pos, epos - pos) *
                      cross(start - pos, end - pos) >= 0)
                i2 = (cross(end - pos, epos - pos) *
                      cross(end - pos, start - pos) >= 0)
                inside = i1 and i2
                if inside:
                    left.append(eitem)
                else:
    ...
    def _reassign_edges_to_left_vertex(self, v: VT, new_g: GraphT,
                                       left_vert: VT,
                                       left_edge_items: list[EItem],
                                       skip_edge_type: Optional[EdgeType] = None) -> None:  # noqa: E501
        """Helper method to reassign edges from original to new left
        vertex.
        ...
        for edge in set(self.graph.incident_edges(v)):
            edge_type = self.graph.edge_type(edge)
            if skip_edge_type is not None and edge_type == skip_edge_type:
                continue
            ...
            for eitem in eitems.values():
                if eitem not in left_edge_items:
                    continue
                edge_type = self.graph.edge_type(edge)
                new_g.add_edge((neighbor, left_vert), edge_type)
                new_g.remove_edge(edge)

    def _finalize_unfuse(self, v: VT, new_g: GraphT) -> None:
        """Helper method to apply animation and push unfuse command to
        undo stack.
        """
    ...
    def _unfuse_w(self, v: VT, left_edge_items: list[EItem],
                  mouse_dir: QPointF) -> None:
        new_g = copy.deepcopy(self.graph)
        ...
        mouse_qpoint = QPointF(self.graph.row(v)/SCALE,
                               self.graph.qubit(v)/SCALE)
        perp_dir = QVector2D(mouse_dir - mouse_qpoint).normalized()
        ...
        left_vert = new_g.add_vertex(VertexType.W_OUTPUT,
                                     qubit=self.graph.qubit(v) + out_offset_y,  # noqa: E501
                                     row=self.graph.row(v) + out_offset_x)
        left_vert_i = new_g.add_vertex(VertexType.W_INPUT,
                                       qubit=self.graph.qubit(v) + in_offset_y,  # noqa: E501
        ...
        # TODO: preserve the edge curve here once it is supported
        # (see https://github.com/zxcalc/zxlive/issues/270)
        self._reassign_edges_to_left_vertex(
            v, new_g, left_vert, left_edge_items,
            skip_edge_type=EdgeType.W_IO)
        self._finalize_unfuse(v, new_g)

    def _unfuse(self, v: VT, left_edge_items: list[EItem],
                right_edge_items: list[EItem], mouse_dir: QPointF,
                phase: Union[FractionLike, complex]) -> None:
        ...
        def compute_avg_vector(pos: QPointF,
                               neighbors: list[EItem]) -> QVector2D:
            avg_vector = QVector2D()
            for eitem in neighbors:
                eitem_x = eitem.selection_node.pos().x()
                eitem_y = eitem.selection_node.pos().y()
                eitem_pos = pos_from_view(eitem_x, eitem_y)
                npos = QPointF(eitem_pos[0], eitem_pos[1])
                direction = QVector2D(npos - pos).normalized()
                avg_vector += direction
        ...
        dist = 0.25 if QVector2D.dotProduct(avg_left, avg_right) != 0 else 0.35  # noqa: E501
        # Put the phase on the left hand side if the mouse direction is
        # closer to the average direction of the left neighbours than the
        # right, i.e., associate the phase to the larger sliced area.
        ...
        # TODO: preserve the edge curve here once it is supported
        # (see https://github.com/zxcalc/zxlive/issues/270)
        self._reassign_edges_to_left_vertex(v, new_g, left_vert,
                                            left_edge_items)
    ...
    def _add_dummy_node(self, x: float, y: float,
                        edges: list[EItem]) -> None:  # noqa: ARG002
        if self.graph_scene.curr_tool != ToolType.VERTEX:
            return
        cmd = ProofModeCommand(
            AddNode(self.graph_view, x, y, VertexType.DUMMY),
            self.step_view)
        self.undo_stack.push(cmd)

    def _add_dummy_edge(self, u: VT, v: VT,
                        verts: list[VItem]) -> None:  # noqa: ARG002
        ...
        if g.type(u) != VertexType.DUMMY or g.type(v) != VertexType.DUMMY:
            return
        cmd = ProofModeCommand(
            AddEdge(self.graph_view, u, v, EdgeType.SIMPLE),
            self.step_view)
        self.undo_stack.push(cmd)

    def delete_selection(self) -> None:
        g = self.graph_scene.g
        rem_vertices = [v for v in self.graph_scene.selected_vertices if
                        g.type(v) == VertexType.DUMMY]
        rem_edges = []
        for e in self.graph_scene.selected_edges:
            s_is_dummy = g.type(g.edge_s(e)) == VertexType.DUMMY
            t_is_dummy = g.type(g.edge_t(e)) == VertexType.DUMMY
            if s_is_dummy and t_is_dummy:
                rem_edges.append(e)
        ...
        if len(set(rem_vertices)) > 128:
            cmd = ProofModeCommand(SetGraph(self.graph_view, new_g),
                                   self.step_view)
        else:
            cmd = ProofModeCommand(UpdateGraph(self.graph_view, new_g),
                                   self.step_view)
        self.undo_stack.push(cmd)

    def paste_graph(self, graph: GraphT) -> None:
        dummy_vertices = [v for v in graph.vertices() if
                          graph.type(v) == VertexType.DUMMY]
        if not dummy_vertices:
            return
        dummy_graph = graph.subgraph_from_vertices(dummy_vertices)
        new_g = copy.deepcopy(self.graph_scene.g)
        new_verts, _ = new_g.merge(dummy_graph.translate(0.5, 0.5))
        cmd = ProofModeCommand(UpdateGraph(self.graph_view, new_g),
                               self.step_view)
        self.undo_stack.push(cmd)
