...
class ZXLive(QApplication):
    ...
    def __init__(self, *, standalone: bool = True) -> None:
        ...
        # Auto-start the editor tutorial on first run (or if the user hasn't
        # opted out).  We pass offer_quick=True so the welcome step includes
        # Quick / Full buttons; the tutorial itself checks the setting before
        # showing anything.
        _tutorial.maybe_start_first_run(main_window)
