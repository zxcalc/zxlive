...
def proof_to_tikz(proof: ProofModel) -> str:
    ...
    max_width = get_settings_value(
        "tikz/layout/max-width", float, settings=settings)
    draw_scalar = False
    ...
    for i, g in enumerate(proof.graphs()):
        ...
        width = (max(g.row(v) for v in g.vertices()) -
                 min(g.row(v) for v in g.vertices()))
        height = (max(g.qubit(v) for v in g.vertices()) -
                  min(g.qubit(v) for v in g.vertices()))
        ...
        if i > 0:
            ...
            rule_key = f"tikz/names/{rewrite.rule}"
            name = (settings.value(rule_key) if settings.contains(rule_key)
                    else rewrite.rule)
            eq = (f"\\node [style=none] ({idoffset}) at "
                  f"({xoffset - hspace/2:.2f}, {-yoffset - height/2:.2f}) "
                  f"{{$\\overset{{\\mathit{{{name}}}}}{{=}}$}};")
