...
def import_diagram_dialog(
        parent: QWidget) -> Optional[
            ImportGraphOutput | ImportProofOutput | ImportRuleOutput]:
...
def create_circuit_dialog(
        explanation: str, example: str, parent: QWidget) -> Optional[str]:
    """Shows a dialog to input a circuit."""
    s, success = QInputDialog.getMultiLineText(
        parent, "Circuit input", explanation, example)
    return s if success else None


def import_diagram_from_file(
        file_path: str, selected_filter: str = FileFormat.All.filter,
        parent: Optional[QWidget] = None) -> Optional[
            ImportGraphOutput | ImportProofOutput | ImportRuleOutput]:
    ...
    if not file.open(
        ...
        show_error_msg(
            f"Could not open file: {file_path}.", parent=parent)
        return None
    ...
    selected_format = next(
        f for f in FileFormat if f.filter == selected_filter)
    if selected_format == FileFormat.All:
        ext = file_path.split(".")[-1]
        try:
            selected_format = next(
                f for f in FileFormat if f.extension == ext)
    ...
    # (requires python 3.10 though)...
    try:
        if selected_format == FileFormat.ZXProof:
            return ImportProofOutput(
                selected_format, file_path, ProofModel.from_json(data))
        elif selected_format == FileFormat.ZXRule:
            return ImportRuleOutput(
                selected_format, file_path, CustomRule.from_json(data))
        elif selected_format in (FileFormat.QGraph, FileFormat.Json):
            g = GraphT.from_json(data)
            if TYPE_CHECKING:
                assert isinstance(g, GraphT)
            g.set_auto_simplify(False)
            # This is something that needs to be better annotated
            # in PyZX
            # type: ignore
            return ImportGraphOutput(selected_format, file_path, g)
        elif selected_format == FileFormat.QASM:
            # We know the return type is Multigraph, but mypy doesn't
            g = Circuit.from_qasm(data).to_graph(
                zh=True, backend='multigraph')  # type: ignore
            if TYPE_CHECKING:
                assert isinstance(g, GraphT)
            g.set_auto_simplify(False)
            # type: ignore
            ...
            try:
                # We know the return type is Multigraph, but mypy doesn't
                g = GraphT.from_tikz(data)  # type: ignore
                if TYPE_CHECKING:
                    assert isinstance(g, GraphT)
                g.set_auto_simplify(False)
                # type: ignore
                ...
                raise ValueError(
                    "Probable reason: attempted to import a proof from "
                    "TikZ, which is not supported.")
        else:
            assert selected_format == FileFormat.All
            try:
                # We know the return type is Multigraph, but mypy doesn't
                g = Circuit.load(file_path).to_graph(
                    zh=True, backend='multigraph')  # type: ignore
                if TYPE_CHECKING:
                    assert isinstance(g, GraphT)
                g.set_auto_simplify(False)
                return ImportGraphOutput(
                    FileFormat.QASM, file_path, g)  # type: ignore
            except TypeError:
                try:
                    g = GraphT.from_json(data)
                    if TYPE_CHECKING:
                    ...
                    g.set_auto_simplify(False)
                    return ImportGraphOutput(
                        FileFormat.QGraph, file_path, g)  # type: ignore
                except Exception:
                    try:
                        # We know the return type is Multigraph,
                        # but mypy doesn't
                        g = GraphT.from_tikz(data)  # type: ignore
                        if TYPE_CHECKING:
                            assert isinstance(g, GraphT)
                        g.set_auto_simplify(False)
                        return ImportGraphOutput(
                            FileFormat.TikZ, file_path, g)  # type: ignore
                    except Exception:
                        show_error_msg(
                            f"Failed to import {selected_format.name} file",
                            f"Couldn't determine filetype: {file_path}.",
                            parent=parent)
                        return None

    except Exception as e:
        show_error_msg(
            f"Failed to import {selected_format.name} file: {file_path}",
            str(e), parent=parent)
        return None
...
def get_file_path_and_format(
        parent: QWidget, filter: str,
        default_input: str = "") -> Optional[tuple[str, FileFormat]]:
    ...
    if selected_format == FileFormat.All:
        try:
            ...
            show_error_msg(
                "Unable to determine file format.", parent=parent)
            return None
...
def save_diagram_dialog(
        graph: GraphT, parent: QWidget) -> Optional[tuple[str, FileFormat]]:
    file_path_and_format = get_file_path_and_format(
        parent, ";;".join([f.filter for f in FileFormat
                           if f != FileFormat.ZXProof]))
    ...
    if selected_format in (FileFormat.QGraph, FileFormat.Json):
        ...
        try:
            ...
            show_error_msg(
                "Failed to convert the diagram to a circuit",
                str(e), parent=parent)
            return None
...
def _save_rule_or_proof_dialog(
        data: str, parent: QWidget, filter: str,
        filename: str = "") -> Optional[tuple[str, FileFormat]]:
    file_path_and_format = get_file_path_and_format(
...
def save_proof_dialog(
        proof_model: ProofModel,
        parent: QWidget) -> Optional[tuple[str, FileFormat]]:
    return _save_rule_or_proof_dialog(
        proof_model.to_json(), parent, FileFormat.ZXProof.filter)


def save_rule_dialog(
        rule: CustomRule, parent: QWidget,
        filename: str = "") -> Optional[tuple[str, FileFormat]]:
    return _save_rule_or_proof_dialog(
        rule.to_json(), parent, FileFormat.ZXRule.filter, filename)


def export_proof_dialog(parent: QWidget) -> Optional[str]:
    file_path_and_format = get_file_path_and_format(
        parent, FileFormat.TikZ.filter)
    if file_path_and_format is None or not file_path_and_format[0]:
        return None
...
def export_gif_dialog(parent: QWidget) -> Optional[str]:
    file_path_and_format = get_file_path_and_format(
        parent, FileFormat.Gif.filter)
    if file_path_and_format is None or not file_path_and_format[0]:
        return None
    return file_path_and_format[0]


def get_lemma_name_and_description(
        parent: MainWindow) -> tuple[Optional[str], Optional[str]]:
...
def create_new_rewrite(parent: MainWindow) -> None:
    ...
    right_button.clicked.connect(
        lambda: get_file(parent, right_button, "right"))
    ...
    def add_rewrite() -> None:
        nonlocal left_graph, right_graph
        if left_graph is None or right_graph is None or name.text() == "" or \
           description.toPlainText() == "":
            return
        rule = CustomRule(
            left_graph, right_graph, name.text(), description.toPlainText())
        try:
            check_rule(rule)
        except Exception as e:
...
def update_dummy_vertex_text(
        parent: QWidget, graph: GraphT, v: VT) -> Optional[GraphT]:
    """Prompt the user for text and return a new graph with the text
    stored in the vertex's vdata under key 'text'.
    If the user cancels, return None. Otherwise, return the new graph with
    the updated vdata.
    """
    if graph.type(v) != VertexType.DUMMY:
        show_error_msg(
            "Invalid Vertex Type",
            "This function can only be used on dummy vertices.",
            parent=parent)
        return None
    current_text = graph.vdata(v, 'text', '')
    input_, ok = QInputDialog.getText(
        parent, "Set Text", "Enter text for dummy node:", text=current_text)
    if not ok:
        return None
...
def show_update_available_dialog(
    ...
    view_release_button = msg.addButton(
