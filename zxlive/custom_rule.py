...
from networkx.algorithms.isomorphism import (
    GraphMatcher, categorical_node_match, categorical_edge_match)
...
class CustomRule:
    def __init__(
            self, lhs_graph: GraphT, rhs_graph: GraphT, name: str,
        ...
        if self.is_rewrite_unfusable:
            self.lhs_graph_without_boundaries_nx = nx.MultiGraph(
                self.lhs_graph_nx.subgraph(
                    [v for v in self.lhs_graph_nx.nodes()
                     if self.lhs_graph_nx.nodes()[v]['type'] !=
                     VertexType.BOUNDARY]))

    def __call__(
            self, graph: GraphT,
            vertices: list[VT]) -> pyzx.rules.RewriteOutputType[VT, ET]:
        if self.is_rewrite_unfusable:
        ...
        graph_matcher = GraphMatcher(
            self.lhs_graph_nx, subgraph_nx,
            node_match=categorical_node_match('type', 1),
            edge_match=categorical_edge_match('type', 1))
        matchings = list(graph_matcher.match())
        matchings = filter_matchings_if_symbolic_compatible(
            matchings, self.lhs_graph_nx, subgraph_nx)
        if len(matchings) == 0:
            raise ValueError("No matchings found")
        matching = matchings[0]
        symbolic_params_map = match_symbolic_parameters(
            matching, self.lhs_graph_nx, subgraph_nx)

        vertices_to_remove = []
        for v in matching:
            if (subgraph_nx.nodes()[matching[v]]['type'] !=
                    VertexType.BOUNDARY):
                vertices_to_remove.append(matching[v])

        boundary_vertex_map: dict[NodeView, int] = {}
        for v in self.rhs_graph_nx.nodes():
            if (self.rhs_graph_nx.nodes()[v]['type'] ==
                    VertexType.BOUNDARY):
                for x, data in self.lhs_graph_nx.nodes(data=True):
                    if (data['type'] == VertexType.BOUNDARY and
                        data['boundary_index'] ==
                            self.rhs_graph_nx.nodes()[v]['boundary_index']):
                        boundary_vertex_map[v] = boundary_mapping[matching[x]]
                        break

        vertex_positions = get_vertex_positions(
            graph, self.rhs_graph_nx, boundary_vertex_map)
        self.last_rewrite_center = np.mean(
            [(graph.row(m), graph.qubit(m))
             for m in boundary_vertex_map.values()], axis=0)
            ...
            if (self.rhs_graph_nx.nodes()[v]['type'] !=
                ...
                if isinstance(phase, Poly):
                    ...
                    if phase.free_vars() == set():
                        phase = (phase.terms[0][0] if len(phase.terms) > 0
                                 else 0)
                vertex_map[v] = graph.add_vertex(
                    ty=self.rhs_graph_nx.nodes()[v]['type'],
    ...
    def unfuse_subgraph_for_rewrite(
            self, graph: GraphT, vertices: list[VT]) -> None:
        def get_adjacent_boundary_vertices(
                g: nx.MultiGraph, v: VT) -> Sequence[VT]:
            return [n for n in g.neighbors(v)
                    if g.nodes()[n]['type'] == VertexType.BOUNDARY]

        subgraph_nx_without_boundaries = nx.MultiGraph(
            to_networkx(graph).subgraph(vertices))
        lhs_vertices = [
            v for v in self.lhs_graph.vertices()
            if self.lhs_graph_nx.nodes()[v]['type'] != VertexType.BOUNDARY]
        lhs_graph_nx = nx.MultiGraph(
            self.lhs_graph_nx.subgraph(lhs_vertices))
        graph_matcher = GraphMatcher(
            lhs_graph_nx, subgraph_nx_without_boundaries,
            node_match=categorical_node_match('type', 1))
        matching = list(graph_matcher.match())[0]

        subgraph_nx, _ = create_subgraph(graph, vertices)
        for v in matching:
            if len(get_adjacent_boundary_vertices(
                    self.lhs_graph_nx, v)) != 1:
                continue
            vtype = self.lhs_graph_nx.nodes()[v]['type']
            outside_verts = get_adjacent_boundary_vertices(
                subgraph_nx, matching[v])
            if (len(outside_verts) == 1 and
                subgraph_nx.get_edge_data(
                    matching[v], outside_verts[0])[0]['type'] ==
                    EdgeType.SIMPLE and
                    vtype != VertexType.W_INPUT):
                continue
            if (vtype == VertexType.Z or vtype == VertexType.X or
            ...
            elif (vtype == VertexType.W_OUTPUT or
                  vtype == VertexType.W_INPUT):
                self.unfuse_w_vertex(graph, subgraph_nx, matching[v], vtype)

    def unfuse_update_edges(
            self, graph: GraphT, subgraph_nx: nx.MultiGraph, old_v: VT,
            new_v: VT) -> None:
    ...
    def unfuse_zx_vertex(
            self, graph: GraphT, subgraph_nx: nx.MultiGraph, v: VT,
            vtype: VertexType) -> None:
        new_v = graph.add_vertex(
            vtype, qubit=graph.qubit(v), row=graph.row(v))
        self.unfuse_update_edges(graph, subgraph_nx, v, new_v)
        graph.add_edge((new_v, v), EdgeType.SIMPLE)

    def unfuse_h_box_vertex(
            self, graph: GraphT, subgraph_nx: nx.MultiGraph, v: VT) -> None:
        new_h = graph.add_vertex(
            VertexType.H_BOX, qubit=graph.qubit(v)+0.3, row=graph.row(v)+0.3)
        new_mid_h = graph.add_vertex(
            VertexType.H_BOX, qubit=graph.qubit(v), row=graph.row(v))
    ...
    def unfuse_w_vertex(
        ...
        new_w_in = graph.add_vertex(
            VertexType.W_INPUT, qubit=graph.qubit(w_in), row=graph.row(w_in))
        new_w_out = graph.add_vertex(
            VertexType.W_OUTPUT, qubit=graph.qubit(w_out),
            row=graph.row(w_out))
    ...
    def matcher(
            self, graph: GraphT,
            in_selection: Callable[[VT], bool]) -> list[VT]:
        ...
        graph_matcher = GraphMatcher(
            lhs_graph_nx, subgraph_nx,
            node_match=categorical_node_match('type', 1),
            edge_match=categorical_edge_match('type', 1))
        matchings = filter_matchings_if_symbolic_compatible(
            list(graph_matcher.match()), lhs_graph_nx, subgraph_nx)
    ...
    def from_json(cls, json_str: Union[str, Dict[str, Any]]) -> "CustomRule":
        if isinstance(json_str, str):
            ...
            d = json_str
        lhs_graph = jsonparser.json_to_graph(
            d['lhs_graph'], 'multigraph')
        rhs_graph = jsonparser.json_to_graph(
            d['rhs_graph'], 'multigraph')
    ...
    def to_rewrite_data(self) -> "RewriteData":
        ...
        return {"text": self.name, "matcher": self.matcher, "rule": self,
                "type": MATCHES_VERTICES, "tooltip": self.description,
                'copy_first': False, 'returns_new_graph': False,
                "custom_rule": True, "lhs": self.lhs_graph,
                "rhs": self.rhs_graph}


def is_rewrite_unfusable(lhs_graph: GraphT) -> bool:
    # if any of the output edges of the lhs_graph is a Hadamard edge,
    # then the rewrite is not unfusable
    ...
    for v in lhs_graph.vertices():
        if lhs_graph.type(v) == VertexType.BOUNDARY:
            continue
        if len([n for n in lhs_graph.neighbors(v)
                if lhs_graph.type(n) == VertexType.BOUNDARY]) > 1:
            return False
    return True


def get_linear(
        v: Poly) -> tuple[Union[int, float, complex, Fraction], Optional[Var],
                          Union[int, float, complex, Fraction]]:
...
def match_symbolic_parameters(
        match: Dict[VT, VT], left: nx.MultiGraph,
        right: nx.MultiGraph) -> Dict[
            Var, Union[int, float, complex, Fraction]]:
    ...
    def update_params(
            v: VT, var: Var, coeff: Union[int, float, complex, Fraction],
            const: Union[int, float, complex, Fraction]) -> None:
...
def filter_matchings_if_symbolic_compatible(
        matchings: list[Dict], left: nx.MultiGraph,
        right: nx.MultiGraph) -> list[Dict]:
    new_matchings = []
...
def to_networkx(graph: GraphT) -> nx.MultiGraph:
    ...
    G.add_edges_from([(source, target, {"type": typ})
                      for source, target, typ in graph.edges()])
    return G


def create_subgraph(
        graph: GraphT,
        verts: list[VT]) -> tuple[nx.MultiGraph, dict[str, int]]:
    ...
    for v in verts:
        for e in graph.incident_edges(v):
            ...
            if s not in verts or t not in verts:
                ...
                boundary_mapping[boundary_node] = (
                    s if s not in verts else t)
                subgraph_nx.add_node(boundary_node, type=VertexType.BOUNDARY)
                subgraph_nx.add_edge(
                    v, boundary_node, type=graph.edge_type(e))
                i += 1
    return subgraph_nx, boundary_mapping


def get_vertex_positions(
        graph: GraphT, rhs_graph: nx.MultiGraph,
        boundary_vertex_map: dict[NodeView, int]) -> dict[
            NodeView, tuple[float, float]]:
    pos_dict = {v: (graph.row(m), graph.qubit(m))
                for v, m in boundary_vertex_map.items()}
    ...
    ret: dict[NodeView, tuple[float, float]] = dict(
        nx.spring_layout(rhs_graph, k=k, pos=pos_dict,
                         fixed=boundary_vertex_map.keys()))
    # if the node type in ret is W_INPUT, move it next to the W_OUTPUT node
    for v in ret:
        if rhs_graph.nodes()[v]['type'] == VertexType.W_INPUT:
            w_out = next((n for n in rhs_graph.neighbors(v)
                          if any(rhs_graph.get_edge_data(v, n, k)['type'] ==
                                 EdgeType.W_IO for k in rhs_graph[v][n])
                          and rhs_graph.nodes()[n]['type'] ==
                          VertexType.W_OUTPUT), None)
            if w_out:
...
def check_rule(rule: CustomRule) -> None:
    ...
    if (len(rule.lhs_graph.inputs()) != len(rule.rhs_graph.inputs()) or
        ...
        raise ValueError(
            "The left-hand side and right-hand side of the rule have "
            "different numbers of inputs or outputs.")
    if (len(rule.lhs_graph.var_registry.vars()) == 0 and
            len(rule.rhs_graph.var_registry.vars()) == 0):
        left_matrix = rule.lhs_graph.to_matrix()
        right_matrix = rule.rhs_graph.to_matrix()
        if not np.allclose(left_matrix, right_matrix):
            if np.allclose(
                    left_matrix / np.linalg.norm(left_matrix),
                    right_matrix / np.linalg.norm(right_matrix)):
                raise ValueError(
        ...
        if not rhs_vars.issubset(lhs_vars):
            ...
            raise ValueError(
                "The right-hand side uses variables not present on "
                f"the left-hand side: "
                f"{', '.join(sorted(str(v) for v in missing))}")
        for vertex in rule.lhs_graph.vertices():
            ...
            if isinstance(phase, Poly):
                try:
                    ...
                    raise ValueError(
                        f"Error in left-hand side phase: {str(e)}")
